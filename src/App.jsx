...
// ══ DATA ══════════════════════════════════════════════════════════════════════
// ── 마지막 데이터 업데이트: 2026-03-10 ─────────────────────────────────────────
const AIRLINES = [
  {code:"EK",name:"Emirates",            hub:"DXB",city:{ko:"두바이",    en:"Dubai"},     status:"주의", affected:true, note:{ko:"95개 노선 운항 중. 100% 복구 목표. 확정 탑승객만 공항 입장. 3/10 드론 공격 이후 일시 중단 후 재개.",en:"95 destinations operating. Targeting 100% recovery. Confirmed bookings only. Brief halt after Mar 10 drone attack, then resumed."}},
  {code:"EY",name:"Etihad Airways",      hub:"AUH",city:{ko:"아부다비",  en:"Abu Dhabi"}, status:"주의", affected:true, note:{ko:"70개 이상 목적지 제한 운항. 3/19까지 축소 일정 지속. 환승 승객은 양쪽 편 모두 운항 확정 시에만 탑승 가능.",en:"70+ destinations on reduced schedule through Mar 19. Transit only if both connecting flights confirmed."}},
  {code:"FZ",name:"flydubai",            hub:"DXB",city:{ko:"두바이",    en:"Dubai"},     status:"주의", affected:true, note:{ko:"바레인·쿠웨이트·도하·다맘 3/20까지, 이라크·레바논·이란 3/28까지 운항 중단. 무스카트↔후자이라 임시 증편 운항.",en:"Bahrain, Kuwait, Doha, Dammam suspended to Mar 20. Iraq, Lebanon, Iran to Mar 28. Extra Muscat↔Fujairah flights added."}},
  {code:"QR",name:"Qatar Airways",       hub:"DOH",city:{ko:"도하",      en:"Doha"},      status:"주의", affected:true, note:{ko:"3/10 도하발 전편 취소. 3/11부터 카이로·런던·서울(ICN)·방콕·쿠알라룸푸르·멜버른 등 재개 예정. 3/22까지 무료 환불·변경 2회.",en:"All Doha flights cancelled Mar 10. Resuming Cairo, London, Seoul(ICN), Bangkok, KL, Melbourne etc from Mar 11. 2 free date changes to Mar 22."}},
  {code:"WY",name:"Oman Air",            hub:"MCT",city:{ko:"무스카트",  en:"Muscat"},    status:"운항중",affected:false,note:{ko:"무스카트 정상 운항. 유럽·아시아 추가편 투입. 중동 우회 핵심 거점 — 좌석 수요 매우 높음.",     en:"Muscat normal ops. Extra flights to Europe & Asia. Key bypass hub — very high demand."}},
  {code:"SV",name:"Saudia",              hub:"RUH",city:{ko:"리야드",    en:"Riyadh"},    status:"주의", affected:true, note:{ko:"리야드·제다 운항 지속. 리야드가 장거리 우회 환승 거점으로 급부상 — 유럽행 좌석 수요 폭증.",    en:"Riyadh & Jeddah ops continuing. Riyadh emerging as key long-haul bypass hub — surging demand for Europe."}},
  {code:"GF",name:"Gulf Air",            hub:"BAH",city:{ko:"바레인",    en:"Bahrain"},   status:"중단", affected:true, note:{ko:"바레인 영공 폐쇄로 전편 운항 중단. BCAA 영공 안전 재개 확인 시 재운항 예정.",              en:"All flights suspended — Bahrain airspace closed. Will resume once BCAA confirms safe reopening."}},
  {code:"RJ",name:"Royal Jordanian",     hub:"AMM",city:{ko:"암만",      en:"Amman"},     status:"주의", affected:true, note:{ko:"3/10~15 암만발 두바이·바레인·도하·다맘·쿠웨이트·코펜하겐·바그다드 편 전부 취소.",          en:"Flights to Dubai, Bahrain, Doha, Dammam, Kuwait, Copenhagen, Baghdad cancelled Mar 10–15."}},
  {code:"ME",name:"Middle East Airlines",hub:"BEY",city:{ko:"베이루트",  en:"Beirut"},    status:"주의", affected:true, note:{ko:"베이루트 축소 운항 유지 중. 레바논 영공 개방이나 인근 불안정.",                             en:"Reduced Beirut operations. Lebanese airspace open but unstable."}},
  {code:"TK",name:"Turkish Airlines",    hub:"IST",city:{ko:"이스탄불",  en:"Istanbul"},  status:"운항중",affected:false,note:{ko:"이스탄불 정상 운항. 중동 노선 발권 승객 5/10까지 무료 변경·환불(2/28 이전 발권).",           en:"Istanbul normal ops. Free change/refund until May 10 for Middle East bookings issued by Feb 28."}},
  {code:"IR",name:"Iran Air",            hub:"IKA",city:{ko:"테헤란",    en:"Tehran"},    status:"중단", affected:true, note:{ko:"국제선 전면 중단. 이란 영공 완전 폐쇄 지속. 출국 불가.",                                 en:"All international flights suspended. Iranian airspace fully closed. No departures possible."}},
  {code:"LY",name:"El Al",              hub:"TLV",city:{ko:"텔아비브",  en:"Tel Aviv"},  status:"주의", affected:true, note:{ko:"이스라엘 영공 PPR 제한. 아테네 등 경유 우회 귀환편 운항 중.",                             en:"Israeli airspace PPR restricted. Diverting via Athens etc. Rescue flights operating."}},
  ...
  {code:"OZ",name:"Asiana Airlines",     hub:"ICN",city:{ko:"인천",      en:"Incheon"},   status:"주의", affected:true, note:{ko:"중동 노선 영향. 하나투어·모두투어 체류객 귀환 지원. 환불 수수료 면제.",                      en:"Middle East routes affected. Korean travel agencies supporting stranded passengers. Refund fee waived."}},
];
const AIRSPACE = [
  {country:{ko:"이란",          en:"Iran"},          status:"폐쇄",note:{ko:"미·이스라엘 공습 이후 민간 영공 전면 폐쇄 지속. OIIX/Tehran FIR 진입 불가. 재개 시점 미정.",        en:"Fully closed to civil aviation since US-Israel strikes. OIIX/Tehran FIR — no entry. No reopening timeline."}},
  {country:{ko:"이라크",        en:"Iraq"},          status:"폐쇄",note:{ko:"ORBB/Baghdad FIR 전면 폐쇄 지속. 72시간 단위 반복 연장 중. 지역 내 마지막으로 재개될 전망.",    en:"ORBB/Baghdad FIR fully closed, extended in 72hr increments. Last airspace in region expected to reopen."}},
  {country:{ko:"바레인·쿠웨이트",en:"Bahrain/Kuwait"},status:"폐쇄",note:{ko:"OBBB/OKAC FIR 전면 폐쇄 지속. Gulf Air 전편 중단. EASA 최고수준 회피 권고.",                      en:"OBBB/OKAC FIR fully closed. Gulf Air all flights suspended. EASA highest-level avoidance advisory."}},
  {country:{ko:"이스라엘",      en:"Israel"},        status:"제한",note:{ko:"LLLL/Tel Aviv FIR 사전허가 필요(PPR). El Al 우회 운항만 가능. EASA 회피 권고.",                    en:"LLLL/Tel Aviv FIR — PPR required. Only El Al diverting. EASA avoidance advisory."}},
  ...
  {country:{ko:"카타르",        en:"Qatar"},         status:"제한",note:{ko:"OTDF/Doha FIR 불안정. 3/10 전편 취소. 3/11부터 제한 재개 예정. 드론 Al Udeid 기지 근처 요격.",      en:"OTDF/Doha FIR unstable. All flights cancelled Mar 10. Limited resumption planned from Mar 11."}},
...
const AIRPORTS = [
  ...
  {iata:"AUH",name:{ko:"자이드 국제공항",          en:"Zayed International"},      city:{ko:"아부다비", en:"Abu Dhabi"},status:"제한",airline:"Etihad Airways",
   detail:{ko:"Etihad 70개 이상 목적지 운항 중. 3/19까지 축소 일정 지속. 환승 승객은 연결 편 양쪽 모두 운항 확정 시에만 허용.",en:"Etihad serving 70+ destinations on reduced schedule until Mar 19. Transit only if both connecting flights confirmed."},
   date:"2026-03-10",
   warn:{ko:"Etihad 앱·웹에서 실시간 편 확인 필수",en:"Verify each flight on Etihad app or website"},site:"https://www.abudhabi-airports.ae"},
  {iata:"DOH",name:{ko:"하마드 국제공항",          en:"Hamad International"},      city:{ko:"도하",     en:"Doha"},     status:"중단",airline:"Qatar Airways",
   detail:{ko:"3/10 도하발 전편 취소. Qatar Airways 3/11부터 카이로·런던·서울(ICN)·방콕·쿠알라룸푸르·멜버른·이스탄불·뭄바이 등 재개 예정. 22일까지 무료 환불·변경 2회.",en:"All Doha departures cancelled Mar 10. Qatar Airways resuming Cairo, London, Seoul(ICN), Bangkok, KL, Melbourne, Istanbul, Mumbai etc from Mar 11. 2 free date changes to Mar 22."},
   date:"2026-03-10",
   warn:{ko:"3/10 전편 취소 — 3/11 재개 전 Qatar 앱 반드시 확인",en:"All flights cancelled Mar 10 — check Qatar app before Mar 11 resumption"},site:"https://dohahamadairport.com"},
...
// ── 최신 뉴스 데이터 (수동 업데이트) ─────────────────────────────────────────
const STATIC_NEWS = [
  {icon:"📢",tag:"Emirates",   severity:"high",  time:"3/10",content:"Emirates, 수일 내 100% 복구 목표. 3/10 드론 공격 후 일시 중단 → 재개. 확정 탑승객만 공항 입장. 84시간 내 두바이(DXB)서 1,140편 출발.",                   source:"emirates.com",    fresh:true},
  {icon:"📢",tag:"Qatar",      severity:"high",  time:"3/10",content:"Qatar Airways, 3/10 도하발 전편 취소. 3/11부터 서울(ICN)·런던·방콕·멜버른·이스탄불 등 재개 예정. 카타르 정부 체류비·비자 연장 지원.",                        source:"qatarairways.com",fresh:true},
  {icon:"📢",tag:"Etihad",     severity:"high",  time:"3/10",content:"Etihad, 70개 이상 목적지 제한 운항. 3/19까지 축소 일정. 2/28 이전 발권 → 3/31까지 무료 재예약 또는 전액 환불.",                                             source:"etihad.com",      fresh:true},
  {icon:"📢",tag:"flydubai",   severity:"high",  time:"3/10",content:"flydubai, 바레인·쿠웨이트·도하·다맘 3/20까지, 이라크·레바논·이란 3/28까지 운항 중단. 무스카트↔후자이라 임시 증편으로 UAE↔오만 연결 유지.",                   source:"flydubai.com",    fresh:true},
  {icon:"⛔",tag:"이라크영공",  severity:"high",  time:"3/10",content:"이라크 영공(ORBB) 72시간 단위 반복 연장 지속. 지역 내 마지막으로 재개될 전망. 이라크 경유 노선 전면 우회 필요.",                                             source:"ICAA/NOTAM",      fresh:true},
  {icon:"⛔",tag:"바레인·쿠웨이트",severity:"high",time:"3/10",content:"바레인(OBBB)·쿠웨이트(OKAC) 영공 전면 폐쇄 지속. Gulf Air 전편 중단. Royal Jordanian 3/10~15 관련 노선 전편 취소.",                                         source:"NOTAM/EASA",      fresh:true},
  {icon:"🚨",tag:"두바이드론",  severity:"high",  time:"3/10",content:"두바이 국제공항(DXB) 터미널 3 인근 드론 공격. 연기·폭발 목격. 수 시간 후 운항 재개. 21,300편 이상 취소(분쟁 이후 누적).",                                    source:"DXB/Flightradar24",fresh:true},
  {icon:"🚨",tag:"항공료급등",  severity:"medium",time:"3/10",content:"제트연료 $4.12/갤런 — 4년래 최고가. 중동 우회로 비행시간 최대 2시간 증가, 운항비 $6,000~7,500/시간 추가. 여름 항공료 인상 전망.",                             source:"TravelPirates",   fresh:true},
  {icon:"✅",tag:"무스카트",   severity:"low",   time:"3/10",content:"무스카트(MCT) 중동 우회 핵심 거점 지속. Oman Air 누적 80편 추가 운항, 97,000명 이송 완료. BA 3/9~12 런던행 구조편 출발 중.",                                   source:"Oman Air / BA",   fresh:true},
  {icon:"📢",tag:"Saudia",     severity:"medium",time:"3/7", content:"Saudia, 리야드·제다 운항 지속. 리야드(RUH)가 유럽행 장거리 우회 환승 거점으로 급부상. 암만·쿠웨이트·아부다비·도하 3/10 23:59 GMT까지 중단.",                   source:"saudia.com",      fresh:false},
  {icon:"📢",tag:"Turkish",    severity:"low",   time:"3/4", content:"Turkish Airlines, 2/28 이전 발권 중동 노선 5/10까지 무료 변경·환불. 이스탄불(IST) 현재 가장 안정적인 경유지.",                                                source:"turkishairlines.com",fresh:false},
